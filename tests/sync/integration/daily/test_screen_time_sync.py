...
class TestParseDurationString:
    ...
    def test_hours_and_minutes(self):
        assert parse_duration_string("3h 41m") == 221.0
        assert parse_duration_string("1h 30m") == 90.0
        assert parse_duration_string("2h 0m") == 120.0

    def test_minutes_only(self):
        assert parse_duration_string("41m") == 41.0
        assert parse_duration_string("5m") == 5.0

    def test_seconds_only(self):
        assert parse_duration_string("59s") == pytest.approx(59 / 60, rel=0.01)
        assert parse_duration_string("30s") == 0.5

    def test_minutes_and_seconds(self):
        assert parse_duration_string("1m30s") == 1.5
        assert parse_duration_string("5m 30s") == 5.5

    def test_hours_only(self):
        assert parse_duration_string("2h") == 120.0

    def test_full_format(self):
        # 1h 30m 30s = 60 + 30 + 0.5 = 90.5
        assert parse_duration_string("1h 30m 30s") == 90.5
    ...
    def test_whitespace_variations(self):
        assert parse_duration_string(" 3h  41m ") == 221.0
        assert parse_duration_string("3h41m") == 221.0


class TestParseActivityLine:
    """Tests for parse_activity_line function."""

    def test_standard_format(self):
        result = parse_activity_line("Netflix (3h 41m)")
        assert result == ("Netflix", 221.0)

    def test_app_with_spaces(self):
        result = parse_activity_line("Brawl Stars (41m)")
        assert result == ("Brawl Stars", 41.0)

    def test_app_with_colon(self):
        result = parse_activity_line("Snapchat: chatta con gli amici (11s)")
    ...
    def test_invalid_format(self):
        assert parse_activity_line("No parentheses") is None
        assert parse_activity_line("Missing (") is None


class TestParseActivityField:
    """Tests for parse_activity_field function."""

    def test_multiple_lines(self):
        activity = "Netflix (3h 41m)\nYouTube (41m)\nInstagram (20m)"
        result = parse_activity_field(activity)
    ...
    def test_empty_field(self):
        assert parse_activity_field(None) == {}
        assert parse_activity_field("") == {}
